 ...
 * 普通4*4
 */
public class Game5 extends GridLayout{
	...
	private Picture1[][] pictureMap = new Picture1[piece][piece];

	public Game5(Context context) {
		super(context);
		initGameView();
	}

	public Game5(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initGameView();
	}

	public Game5(Context context, AttributeSet attrs) {
		super(context, attrs);
		initGameView();
	}
	...
	protected void addpicture(int pictureWidth, int pictureHeight) {
		...
		for (int x = 0; x < piece; x++) {
			for (int y = 0; y < piece; y++) {
				picture = new Picture1(getContext());
				a++;
				picture.setNum(a);
		...
		for (int i = 0; i < 20; i++) {
			...
			float c = ((int) (Math.random() * piece)) * Game5.pictureWidth;
			switch (b) {
			case 1:
				swipeDown(c);
				break;
