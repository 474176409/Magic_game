...
import com.upc.mofang.utils.Picture1;
import com.upc.mofang.utils.Picture2;
 ...
 * 4*4有方向
 */
public class Game3 extends GridLayout{
	public int piece = 4;// 几乘几
	// 每张图片的宽度
	...
	private Picture2[][] pictureMap = new Picture2[piece][piece];

	public Game3(Context context) {
		super(context);
		initGameView();
	}

	public Game3(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initGameView();
	}

	public Game3(Context context, AttributeSet attrs) {
		super(context, attrs);
		initGameView();
	}
	...
	public void startGame() {
		GameFour.getGameFour().clearType();
	}
	...
	protected void addpicture(int pictureWidth, int pictureHeight) {
		...
		for (int x = 0; x < piece; x++) {
			for (int y = 0; y < piece; y++) {
				picture = new Picture2(getContext());
				a++;
				picture.setNum(a);
		...
		for (int i = 0; i < 20; i++) {
			...
			float c = ((int) (Math.random() * piece)) * Game3.pictureWidth;
			switch (b) {
			case 1:
				swipeDown(c);
				break;
	...
	private void swipeUp(float startX) {
		GameFour.getGameFour().addType(1);
		int num;
...
//			pictureMap[0][0].setNum(pictureMap[3][0].getNum());
...
//			break;
		case 1:// 第二列
			GameFour.getGameFour().addType(1);
			num = pictureMap[0][1].getNum();
			pictureMap[0][1].setNum(pictureMap[3][1].getNum());
		...
		case 2:// 第三列
			GameFour.getGameFour().addType(1);
			num = pictureMap[0][2].getNum();
			pictureMap[0][2].setNum(pictureMap[3][2].getNum());
...
//		case 3:// 第四列
//			num = pictureMap[0][3].getNum();
//			pictureMap[0][3].setNum(pictureMap[3][3].getNum());
...
//		case 0:// 第一行
//			num = pictureMap[0][0].getNum();
//			pictureMap[0][0].setNum(pictureMap[0][3].getNum());
...
//			break;
		case 1:// 第二行
			GameFour.getGameFour().addType(1);
			num = pictureMap[1][0].getNum();
			pictureMap[1][0].setNum(pictureMap[1][3].getNum());
		...
		case 2:// 第三行
			GameFour.getGameFour().addType(1);
			num = pictureMap[2][0].getNum();
			pictureMap[2][0].setNum(pictureMap[2][3].getNum());
...
//		case 3:// 第三行
//			num = pictureMap[3][0].getNum();
//			pictureMap[3][0].setNum(pictureMap[3][3].getNum());
