 ...
 * 5*5
 */
public class Game6 extends GridLayout{
	public int piece = 5;// 几乘几
	// 每张图片的宽度
	...
	private Picture4[][] pictureMap = new Picture4[piece][piece];

	public Game6(Context context) {
		super(context);
		initGameView();
	}

	public Game6(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		initGameView();
	}

	public Game6(Context context, AttributeSet attrs) {
		super(context, attrs);
		initGameView();
	}
	...
	public void startGame() {
		GameSeven.getGameSeven().clearType();
	}
	...
	protected void addpicture(int pictureWidth, int pictureHeight) {
		...
		for (int x = 0; x < piece; x++) {
			for (int y = 0; y < piece; y++) {
				picture = new Picture4(getContext());
				a++;
				picture.setNum(a);
		...
		for (int i = 0; i < 20; i++) {
			...
			float c = ((int) (Math.random() * piece)) * Game6.pictureWidth;
			switch (b) {
			case 1:
				swipeDown(c);
				break;
	...
	private void swipeUp(float startX) {
		GameSeven.getGameSeven().addType(1);
		int num;
	...
	private void swipeDown(float startX) {
		...
		case 0:// 第一列
			num = pictureMap[0][0].getNum();
			pictureMap[0][0].setNum(pictureMap[4][0].getNum());
		...
		case 1:// 第二列
			num = pictureMap[0][1].getNum();
			pictureMap[0][1].setNum(pictureMap[4][1].getNum());
		...
		case 2:// 第三列
			num = pictureMap[0][2].getNum();
			pictureMap[0][2].setNum(pictureMap[4][2].getNum());
		...
		case 3:// 第四列
			num = pictureMap[0][3].getNum();
			pictureMap[0][3].setNum(pictureMap[4][3].getNum());
		...
		case 4:// 第五列
			num = pictureMap[0][4].getNum();
			pictureMap[0][4].setNum(pictureMap[4][4].getNum());
	...
	private void swipeRight(float startY) {
		...
		case 0:// 第一行
			num = pictureMap[0][0].getNum();
			pictureMap[0][0].setNum(pictureMap[0][4].getNum());
		...
		case 1:// 第二行
			num = pictureMap[1][0].getNum();
			pictureMap[1][0].setNum(pictureMap[1][4].getNum());
		...
		case 2:// 第三行
			num = pictureMap[2][0].getNum();
			pictureMap[2][0].setNum(pictureMap[2][4].getNum());
		...
		case 3:// 第三行
			num = pictureMap[3][0].getNum();
			pictureMap[3][0].setNum(pictureMap[3][4].getNum());
		...
		case 4:// 第三行
			num = pictureMap[4][0].getNum();
			pictureMap[4][0].setNum(pictureMap[4][4].getNum());
